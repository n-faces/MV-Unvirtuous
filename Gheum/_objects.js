...
class Game_System {
    ...
    isTitleBarEnabled() {
        return this._titleBarEnabled;
    };

    disableTitleBar() {
        this._titleBarEnabled = false;
    };
...
class Game_Battler extends Game_BattlerBase {
    ...
    getSheetData() {
        DataManager.loadSheetDataFile(this);
    }
...
class Game_Map {
    ...
    setupObjectives() {
        this._objectives = [];
        for (var i = 0; i < $dataMap.objectives.length; i++) {
...
// Change Menu Access
    command135() {
        if (this._params[0] === 0) {
            $gameSystem.disableTitleBar();
        } else {
            $gameSystem.enableTitleBar();
        }
        return true;
    };

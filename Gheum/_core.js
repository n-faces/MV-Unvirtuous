...
class Random {
    ...
    static choice(choices) {
        return choices[this.range(0, choices.length - 1)];
    };
...
//-----------------------------------------------------------------------------
/**
 * The static class containing additional methods for Math object
 *
 * @static
 * @class MathHelper
 */
...
class Graphics {
    ...
    static _createRenderer() {
        ...
        try {
            ...
            if (this._renderer) this._renderer.backgroundColor = this._getBackgroundColor();
    ...
    static _getBackgroundColor() {
        var a = document.body.style.backgroundColor.split('(')[1].split(')')[0].split(',');
        var b = a.map(function (x) {             //For each array element
            x = parseInt(x).toString(16);      //Convert to a base16 string
            return (x.length === 1) ? '0' + x : x;  //Add zero if we get only one character
        });
        return parseInt(b.join(''), 16);
    }
...
class Window extends PIXI.Container {
     ...
     */
    _updateCursor() {
        this._windowCursorSprite.alpha = 0.6;
...
class WebAudio {
    ...
    _readLittleEndian(array, index) {
        return (array[index + 3] * 0x1000000 + array[index + 2] * 0x10000 +
            array[index + 1] * 0x100 + array[index]);
    };
    ...
    _readBigEndian(array, index) {
        return (array[index] * 0x1000000 + array[index + 1] * 0x10000 +
...
class JsonEx {
    ...
    /**
     * Iterate all the properties of the object.
     *
     * @static
     * @method flatten

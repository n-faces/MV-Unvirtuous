...
class Window_Base extends Window {
    ...
    loadWindowskin() {
        this.windowskin = ImageManager.loadSystem('Window_Gh');
    };
...
function Window_TitleBar() {
    this.initialize.apply(this, arguments);
}

Window_TitleBar._iconWidth = 32;
Window_TitleBar._iconHeight = 32;

Window_TitleBar.prototype = Object.create(Window_HorzCommand.prototype);
Window_TitleBar.prototype.constructor = Window_TitleBar;

Window_TitleBar.prototype.initialize = function() {
    this._updateArrows = this._refreshArrows = function() {};
    this._updateCursor = this._refreshCursor = function() {};
    this.updateCursor = function() {};
    var sx = Graphics.boxWidth - this.windowWidth();
...
Window_TitleBar.prototype.standardPadding = function() {
    return 8;
};

Window_TitleBar.prototype.windowWidth = function() {
    return 768;
};

Window_TitleBar.prototype.windowHeight = function() {
    return (Window_TitleBar._iconHeight + this.standardPadding() * 2) + 8;
};

Window_TitleBar.prototype.spacing = function() {
    return 6;
};

Window_TitleBar.prototype.maxRows = function() {
    return 1;
};

Window_TitleBar.prototype.maxCols = function() {
    return 3;
};

Window_TitleBar.prototype.itemRect = function(index) {
    var rect = new Rectangle();
    rect.width = this.itemWidth();
    rect.height = this.itemHeight();
...
Window_TitleBar.prototype.makeCommandList = function() {
    this.addCommand('Menu', 'options');
    this.addCommand('Test Mode', 'test');
    this.addCommand('Exit', 'exit');
};

Window_TitleBar.prototype.setAllHandlers = function() {
    this.setHandler('test', this.commandTestMode.bind(this));
    this.setHandler('exit', this.commandExit.bind(this));
};

Window_TitleBar.prototype.commandTestMode = function() {
    if (Utils.isNwjs() && Utils.isOptionValid('test')) {
        history.replaceState(null, '', location.href.replace('Gheum/Gheum', 'index'));
        location.reload();
    }
};

Window_TitleBar.prototype.commandExit = function() {
    this.close();
    SceneManager.exit();
};

Window_TitleBar.prototype.update = function() {
    Window_Base.prototype.update.call(this);
    this.processHandling();
    this.processTouch();
    this._stayCount++;
...
Window_TitleBar.prototype.isCurrentItemEnabled = function() {
    return $gameSystem.isTitleBarEnabled();
};

Window_TitleBar.prototype.processHandling = function() {
    if (this.isOpenAndActive()) {
        if (this.isTitleBarCalled('options')) {
            this.processTitleBar('options');
        } else if (this.isTitleBarCalled('test')) {
            this.processTitleBar('test');
        } else if (this.isTitleBarCalled('exit')) {
            this.processTitleBar('exit');
        } else if (this.isCancelEnabled() && this.isCancelTriggered()) {
            this.processCancel();
        }
    }
...
Window_TitleBar.prototype.isTitleBarCalled = function(name) {
    return Input.isRepeated(name);
};

Window_TitleBar.prototype.processTitleBar = function(name) {
    if (this.isCurrentItemEnabled()) {
        this.updateInputData();
...
Window_TitleBar.prototype.onTouch = function(triggered) {
    var lastIndex = this.index;
    var x = this.canvasToLocalX(TouchInput.x);
    var y = this.canvasToLocalY(TouchInput.y);
    var hitIndex = this.hitTest(x, y);
...
Window_Menu.prototype.standardPadding = function() {
    return 44;
};
...
Window_Menu.prototype.makeCommandList = function() {
    ...
    this.addSaveCommand();
    this.addToTitleCommand();
    this.addCommand('Continue', 'continue', true);
};

Window_Menu.prototype.updatePlacement = function() {
...
Window_Menu.prototype.addToTitleCommand = function() {
    var enabled = this.isToTitleEnabled();
    this.addCommand('To Title', 'toTitle', enabled);
};

Window_Menu.prototype.needsCommand = function(name) {
...
Window_Menu.prototype.isToTitleEnabled = function() {
    return true;
};
...
Window_Objectives.prototype.standardPadding = function() {
    return 44;
};
...
Window_Status.prototype.drawExpInfo = function(x, y) {
    ...
    this.changeTextColor(this.systemColor());
    this.drawText(expTotal, x, y, 270);
...
Window_MapName.prototype.initialize = function() {
    var width = this.windowWidth();
    var height = this.windowHeight();
    var sx = (Graphics.boxWidth - width)/2;
    var sy = Graphics.boxHeight * 0.5 - 320;
...
Window_CharacterBars.prototype.standardFontSize = function() {
    return 16;
};

Window_CharacterBars.prototype.drawText = function(text, x, y, maxWidth, align) {
    this.contents.drawText(text, x, y - this.standardPadding() / 2, maxWidth, this.lineHeight(), align);
};

Window_CharacterBars.prototype.fittingHeight = function(numLines) {
    return numLines * (this.lineHeight() + Window_CharacterBars.barSpacing) + this.standardPadding() * 2;
};
...
Window_PlayerBars.prototype.drawBasicArea = function(rect, actor) {
    var d = Window_CharacterBars.faceLength;
...
Window_BattleActor.prototype.cursorDown = function() {
    var index = this.index;
...
Window_BattleEnemy.prototype.isNextEnemyInFront = function(index) {
...
// Find a way to trigger frontline-filter
...
Window_BattleEnemy.prototype.cursorDown = function() {
    ...
    if (this.isNextEnemyInFront(next)) {
        this.select(next);
    }
    if (!this.enemy()) {
        this.cursorDown();
...
Window_BattleEnemy.prototype.cursorUp = function() {
    var index = this.index;
    var next = function(index) {
        return (index - 1).mod(3) + (Math.floor(index / 3) * 3);
    }(index);
...
Window_MenuBar.prototype = Object.create(Window_Command.prototype);
...
Window_MenuBar._lastCommandSymbol = null;

Window_MenuBar.initCommandPosition = function() {
    this._lastCommandSymbol = null;
};
...
Window_MenuBar.prototype.itemTextAlign = function() {
    return 'center';
};

Window_MenuBar.prototype.updatePlacement = function() {
    this.x = 0;
    this.y = Graphics.boxHeight - this.height - 96;
};

Window_MenuBar.prototype.makeCommandList = function() {
    this.addCommand('Players', 'players');
    this.addCommand('Classes', 'classes');
    this.addCommand('Skills', 'skills');
    this.addCommand('Items', 'items');
    this.addCommand('Weapons', 'weapons');
    this.addCommand('Armors', 'armors');
    this.addCommand('Enemies', 'enemies');
    this.addCommand('Troops', 'troops');
    this.addCommand('States', 'states');
    this.addCommand('Animations', 'animations');
    this.addCommand('Tilesets', 'tilesets');
    this.addCommand('Common Events', 'commons');
    this.addCommand('Systems', 'systems');
    this.addCommand('Types', 'types');
    this.addCommand('Terms', 'terms');
};

Window_MenuBar.prototype.processOk = function() {
    Window_MenuBar._lastCommandSymbol = this.currentSymbol();
    if (this.isCurrentItemEnabled()) {
        this.updateInputData();
        this.callOkHandler();
    }
...
Window_MenuBar.prototype.selectLast = function() {
    if (Window_MenuBar._lastCommandSymbol) {
        this.selectSymbol(Window_MenuBar._lastCommandSymbol);
    }
};
...
Window_GameEnd.prototype.standardPadding = function() {
    return 44;
};
...
function Window_TroopButton() {
    this.initialize.apply(this, arguments);
}

Window_TroopButton.prototype = Object.create(Window_Command.prototype);
Window_TroopButton.construcotr = Window_TroopButton;

Window_TroopButton.prototype.initialize = function() {
    Window_Command.prototype.initialize.call(this, 0, 0);
};

Window_TroopButton.prototype.makeCommandList = function() {
    this.addCommand('Troop', 'troop');
    this.addCommand('Skill Tree', 'skillTree');
    this.addCommand('System', 'system');
};

Window_TroopButton.prototype.windowWidth = function(){
    return 200;
};

Window_TroopButton.prototype.itemTextAlign = function(){
    return 'center';
};

function Window_TroopPreview() {
    this.initialize.apply(this, arguments);
}

Window_TroopPreview.prototype = Object.create(Window_Base.prototype);
Window_TroopPreview.constructor = Window_TroopPreview;

Window_TroopPreview.prototype.initialize = function() {
    Window_Base.prototype.initialize.call(this, 0, 0, 400, 400);
    this._text = '';
};

function Window_TroopName() {
    this.initialize.apply(this, arguments);
}

Window_TroopName.prototype = Object.create(Window_Command.prototype);
Window_TroopName.constructor = Window_TroopName;

Window_TroopName.prototype.initialize = function() {
    Window_Command.prototype.initialize.call(this, 0, 0);
};

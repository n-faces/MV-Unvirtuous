...
class DataManager {
    ...
    static loadSheetDataFile(receptor) {
        var xhr = new XMLHttpRequest();
        var sv = (receptor instanceof Game_Actor) ? 'sv_actors/' : 'sv_enemies/';
        var url = '../img/' + sv + receptor.battlerName() + '.json' ;
        xhr.open('GET', url);
        xhr.overrideMimeType('application/json');
        xhr.onload = function () {
            if (xhr.status < 400) {
                receptor._sheetData = JSON.parse(xhr.responseText);
                DataManager.onLoad(receptor._sheetData);
            }
        };
